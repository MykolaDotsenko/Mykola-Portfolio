const ExperienceTopRight = () => {
  return (
    <div className="xl:w-[25%] lg:w-[30%] border border-lightBrown p-4 rounded-xl">
      <p className="text-lg text-center text-lightGrey">
        I specialize in{" "}
        <span className="font-bold text-white">
          React, Next.js, and modern JavaScript frameworks
        </span>
        , leveraging best practices to build scalable and maintainable
        solutions. <br />
        My experience spans working on diverse projects, from small business
        websites to{" "}
        <span className="font-bold text-white">complex full-stack systems</span>,
        always aiming for clean code and exceptional user experiences. I thrive on challenges that push me to grow.
      </p>
    </div>
  );
};

export default ExperienceTopRight;
